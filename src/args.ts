...
export function parseArgs(args: string[]): CliOptions {
  ...
  let connectionOverridden = false;
  ...
  while (i < args.length) {
    if (args[i] === '--format' && args[i + 1]) {
      ...
      filter = args[i + 1];
      i += 2;
    } else if (args[i] === '--connection' && args[i + 1]) {
      connection = args[i + 1];
  ...
  const config: SpecConfig = {
    ...
    ...(connection !== undefined ? { sql: { connection } } : {}),
  };

  return { command, targets, format, config, filter, baseOverridden, connectionOverridden, verbose };
}
